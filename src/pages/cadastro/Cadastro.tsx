...
function Cadastro() {
  ...
  async function cadastrarNovoUsuario(e: FormEvent<HTMLFormElement>) {
    ...
    if (!usuario.tipo) {
      alert(
        "Por favor, selecione o tipo de usuário (Admin, Médico ou Assistente)."
      );
      return;
    }
    ...
    if (confirmarSenha === usuario.senha && usuario.senha.length >= 8) {
      ...
      try {
        ...
        alert(
          "Erro ao cadastrar o usuário! Verifique as informações fornecidas."
        );
      }
    } else {
      alert(
  ...
  const precisaDeCrm = usuario.tipo === 'medico' || usuario.tipo === 'admin';
  const crmPreenchido = usuario.crm && usuario.crm.trim().length > 0;
  const isFormInvalid = (precisaDeCrm && !crmPreenchido) || 
        ...
        <div className="flex justify-center items-center w-full p-4 animate-fade-in">
          ...
          >
            <div className="text-center">
              <h2 className="text-[#45C4B0] text-5xl font-black tracking-tight mb-2 italic">
                CADASTRO
              </h2>
              <p className="text-[#9AEBA3] text-sm font-light tracking-widest uppercase">
                Crie sua conta
              </p>
            </div>
            ...
            <div className="space-y-4 mt-4">
              <div className="flex flex-col gap-1.5 group">
                <label
                  htmlFor="nome"
                  className="text-white/70 text-xs font-bold uppercase ml-1 group-focus-within:text-[#45C4B0] transition-colors"
                >
                  Nome
                </label>
                <input
              ...
              <div className="flex flex-col gap-1.5 group">
                ...
                >
                  ...
                  <option value="MEDICO" className="text-black">
                    Médico
                  </option>
                  <option value="ASSISTENTE" className="text-black">
                    Assistente
                  </option>
                </select>
              </div>
              ...
              {(usuario.tipo === "medico" || usuario.tipo === "admin") && (
                <div className="flex flex-col gap-1.5 group animate-fade-in">
                  <label
                    htmlFor="crm"
                    className="text-white/70 text-xs font-bold uppercase ml-1 group-focus-within:text-[#45C4B0] transition-colors"
                  >
            ...
            <div className="flex flex-col sm:flex-row justify-around w-full gap-4 mt-6">
             ...
             ${
               isFormInvalid
                 ? "bg-gray-500 cursor-not-allowed opacity-50 shadow-none transform-none"
                 : "bg-[#45C4B0] hover:bg-[#9AEBA3] shadow-[0_0_15px_rgba(69,196,176,0.4)] hover:shadow-[0_0_25px_rgba(154,235,163,0.6)] hover:-translate-y-1"
             }`}
              >
                {isLoading ? (
                  <ClipLoader color="#012340" size={20} />
                ) : (
                  <span>Cadastrar</span>
                )}
              </button>
