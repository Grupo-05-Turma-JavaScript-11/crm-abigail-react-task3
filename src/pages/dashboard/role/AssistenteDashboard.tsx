...
function mapStatus(raw: any): AgendaItem["status"] {
  ...
  if (s.includes("atend")) return "EM TRATAMENTO";
...
export default function AssistenteDashboard() {
  ...
  const [periodoAtivo, setPeriodoAtivo] = useState<"Dia" | "Semana" | "Mês">(
    "Dia"
  );

  useEffect(() => {
    buscar("/atendimentos", setRawAtendimentos, {
  ...
  const agenda: AgendaItem[] = useMemo(() => {
    return rawAtendimentos.map((a: any) => {
      const date = safeDate(
        a.dataHora ?? a.data_hora ?? a.data ?? a.horario ?? a.dateTime
      );
  ...
  const statCards: StatCard[] = useMemo(() => {
    ...
    const confirmados = hoje.filter((a: any) =>
      String(a.status ?? "")
        .toLowerCase()
        .includes("confirm")
    );
    const emAtendimento = hoje.filter((a: any) =>
      String(a.status ?? "")
        .toLowerCase()
        .includes("atend")
    );
    ...
    return [
      ...
      },
      {
        key: "conf",
        title: "Confirmados",
        value: String(confirmados.length),
        note: "hoje",
        accent: "green",
        icon: <span />,
      },
      {
        ...
        value: String(emAtendimento.length),
        note: "agora",
        accent: "aqua",
        icon: <span />,
      },
